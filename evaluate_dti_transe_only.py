"""
Drug-Target Interaction (DTI) Evaluation Script for TransE Only
==============================================================
Compute AUC-ROC and AUC-PR for DRUG_TARGET prediction using only the TransE model.

This script:
1. Loads the trained TransE model
...
OUTPUT_DIR = "outputs_dti_evaluation_transe"
...
def plot_roc_curve(y_true, y_scores, save_path):
    ...
    plt.plot(fpr, tpr, label=f'TransE (AUC = {auc:.4f})', color='#3498db', linewidth=2)
    plt.plot([0, 1], [0, 1], 'k--', linewidth=1, label='Random')
    plt.xlabel('False Positive Rate', fontsize=12)
    plt.ylabel('True Positive Rate', fontsize=12)
    plt.title('ROC Curve for Drug-Target Prediction (TransE)', fontsize=14, fontweight='bold')
...
def plot_pr_curve(y_true, y_scores, save_path):
    ...
    plt.figure(figsize=(8, 8))
    plt.plot(recall, precision, label=f'TransE (AP = {ap:.4f})', color='#3498db', linewidth=2)
    plt.axhline(y=baseline, color='k', linestyle='--', linewidth=1, label=f'Random (AP = {baseline:.4f})')
    plt.xlabel('Recall', fontsize=12)
    plt.ylabel('Precision', fontsize=12)
    plt.title('Precision-Recall Curve for Drug-Target Prediction (TransE)', fontsize=14, fontweight='bold')
...
def main():
    print("=" * 70)
    print("DRUG-TARGET INTERACTION PREDICTION EVALUATION (TransE Only)")
    ...
    if not os.path.exists(MODEL_PATH):
        print(f"\n⚠️ TransE model not found at {MODEL_PATH}")
        return
    ...
    df.to_csv(os.path.join(OUTPUT_DIR, f"dti_metrics_transe_neg{NEG_RATIO}.csv"), index=False)
    print(f"\nSaved: {os.path.join(OUTPUT_DIR, f'dti_metrics_transe_neg{NEG_RATIO}.csv')}")
    # Generate plots
    print("\n🎨 Generating plots...")
    plot_roc_curve(y_true, y_scores, os.path.join(OUTPUT_DIR, "figures", f"roc_curve_transe_neg{NEG_RATIO}.png"))
    plot_pr_curve(y_true, y_scores, os.path.join(OUTPUT_DIR, "figures", f"pr_curve_transe_neg{NEG_RATIO}.png"))
    # Save detailed report
    with open(os.path.join(OUTPUT_DIR, f"dti_evaluation_report_transe_neg{NEG_RATIO}.txt"), "w") as f:
        f.write("DRUG-TARGET INTERACTION PREDICTION EVALUATION (TransE)\n")
        f.write("=" * 60 + "\n\n")
    ...
    print(f"Saved: {os.path.join(OUTPUT_DIR, f'dti_evaluation_report_transe_neg{NEG_RATIO}.txt')}")
    print("\n" + "=" * 70)
    print("✅ DTI Evaluation Complete! (TransE)")
    print("=" * 70)
